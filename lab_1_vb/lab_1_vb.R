...
d.counties %>%
  ...
  geom_point() +
  theme_classic() +
  labs(title = "Relationship between Land and Water in CBW Counties", x = "Land Area", y = "Water Area", color = "State")
...
d.stations %>%
  ...
  theme_classic() +
  labs(title = "Drainage Area of CWB Monitoring Stations", x = "Drainage Area", y = "Station Count")
...
d.stations %>%
  ...
  geom_histogram(binwidth = 1000) +
  theme_classic() +
  labs(title = "Drainage Area of CWB Monitoring Stations", x = "Drainage Area", y = "Station Count", fill = "State")
...
f.3.1 <- function(x) {
  ...
  vector.min <- min(vector)
  vector.sorted <- sort(vector)
  return(list(mean = vector.mean, median = vector.median, max = vector.max, min = vector.min, sorted = vector.sorted))
}
...
d.counties %>%
  filter(STATEFP10 == 36) %>%
  summarise(avg_size_NY = mean(ALAND10 + AWATER10))
...
#A1: This intersection is between a set of polygons (counties) and a set of points (stations), so in theory, the order of the variables within the function should not matter geometrically when performing the intersection. However, the variables contain different attributes and different geometries, so their order in the function will change the output of the intersection in R. For example, the attributes of the first argument will be referenced first, then the second argument will be overlaid and some of its attributes can be lost if they conflict with the first argument. Likewise, the geometry of the first argument determines the geometry of the output, so if the first variable has point geometry then the output will too (or vice versa). The same is true if you try to perform an intersection with other types of spatial data like lines and polygons.

#Q2: What did you find challenging in this lab? What was new?
#A2: I felt like this lab had a steep learning curve at the beginning due to my lack of programming experience. For instance, things that should have been easy like loading in the data took me awhile, but once I got familiar with the structure of the data and the syntax of the code, I was able to work my way through the tasks without too much difficulty. In the end, it was satisfying to solve a lot of these problems and I am learning the foundations to write cleaner and more complex code moving forward. 

#Q3: What types of activities would you like to see in labs this semester?
#A3: There is not any particular type of analysis I am looking to learn this semester because I am new to R and trying to become familiar with the different types of spatial analysis available that I can potentially apply to my research. I want to gain experience working with big data, especially related to water quality (which we have already started) and ideally data related to wildlife movement/habitat use as well. In terms of data visualization, I would like to practice incorporating satellite imagery into maps for more realism and context.
